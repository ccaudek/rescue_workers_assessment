...
m10 <- brm(
  ...
  family = brmsfamily("hurdle_gamma"),
  backend = "cmdstanr",
  all_items
)
```
...
me <- conditional_effects(
  m10, "is_rescue_worker"
)
plot(me, points = FALSE)
```

The factor model does not fit very well, but the Omega for each subscale is
acceptable.

```{r}
iesr_df <- all_items |> 
  dplyr::select(starts_with("ies_"))
```


```{r}
model <- '
    intrusion =~ ies_5 + ies_4 + ies_7 + ies_6 + ies_3 + ies_1
    avoidance =~ ies_18 + ies_17 + ies_22 + ies_21 + ies_16 + ies_19 + ies_15
    hyperarousal =~ ies_12 + ies_11 + ies_9 + ies_10 + ies_13
...
```{r}
fit <- cfa(model, data = iesr_df, std.lv = TRUE)
...
```{r}
cope_scales <- c(
  "pos_reinterpretation", "active_coping", "restraint", "acceptance",
                  "suppr_competing_activities", "planning", "venting", 
  "seeking_instrumental_support", "religion", 
                   "seeking_emotional_support", "mental_disengagement", "denial", "humor",
                     "behavioral_disengagement", "substance_use"
)

cope_df <- subset(all_items, select=cope_scales)


fit0 <- factanal(cope_df, 3, rotation="promax")
print(fit0, digits=2, cutoff=0.3, sort=TRUE)
```
...
```{r}
semTools::reliability(fit1, return.total = TRUE)
```
...
```{r}
t.test(active_coping ~ is_rescue_worker, data= all_items)
```

```{r}
t.test(avoidance_coping ~ is_rescue_worker, data= all_items)
```


```{r}
t.test(soc_emo_coping ~ is_rescue_worker, data = all_items)
```
...
```{r}
fit2 <- sem(model2_cope, data = all_items)
...
```{r}
fm <- lm(avoidance_coping ~ ts_sc * is_rescue_worker, all_items)
summary(fm)
```

```{r}
plot(density(all_items$avoidance_coping))
```
```{r}
library("brms")
```
...
```{r}
fit_2 <- brm(
  active_coping ~ ts_sc * is_rescue_worker,
  family = asym_laplace(),
  backend = "cmdstanr",
  data = all_items
)
...
```{r}
# all_items$active_coping <- all_items$pos_reinterpretation + 
#   all_items$active_coping + all_items$suppr_competing_activities + 
#   all_items$planning + all_items$restraint + 
#   all_items$seeking_instrumental_support
# 
# all_items$avoidance_coping <- all_items$mental_disengagement + all_items$denial + 
#   all_items$humor + all_items$behavioral_disengagement + 
...
cutoff <- (qchisq(p = 1 - .001, df = ncol(temp1)))
# lpa_clean <- temp1 %>%
#   filter(string <= 10,
#          md < cutoff) %>%
#   select(-string, -md)
temp1$FLAG <- ifelse(
  temp1$string > 10 | temp1$md > cutoff, 1, 0
)

lpa_clean <- lpa_df
lpa_clean$FLAG <- temp1$FLAG
...
```{r}
is_rescue_worker_vector <- lpa_clean$is_rescue_worker
lpa_clean$is_rescue_worker <- NULL
```
...
Compare tidyLPA solutions:
 ...
 1     1       121911.101 122079.946
 1     2       119241.918 119500.152
 1     3       118697.545 119045.167
 1     4       118115.937 118552.947
 1     5       117629.426 118155.825
 1     6       116990.674 117606.461
 1     7       116563.688 117268.864
 1     8       116380.951 117175.514
 1     9       116278.138 117162.091
 1     10      116146.253 117119.594
 6     1       115441.794 116286.018
 6     2       114456.657 116150.071
 6     3       114165.869 116708.473
 6     4       114039.472 117431.267
 6     5       113986.612 118227.596
 6     6       113940.600 119030.775
 6     7       114012.142 119951.507
 6     8       114128.308 120916.863
 6     9       114039.141 121676.886
 6     10      114170.629 122657.564
...
```{r}
out <- get_data(m2)
lpa_clean$lpa_class <- out$Class
```

```{r}
table(
  lpa_clean$lpa_class, is_rescue_worker_vector
)
```
