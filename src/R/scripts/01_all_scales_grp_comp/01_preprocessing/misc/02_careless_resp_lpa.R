...
classes_one_five <- by_subj_vals %>%
  dplyr::filter(Class == 1 | Class == 1) %>%
  dplyr::select(!(starts_with("CPROB")))

classes_one_five %>%
  as.data.frame()

bad_participants <- classes_one_five$index
length(bad_participants)

items_clean <- all_items[-bad_participants, ]

# Write cleaned items to the "processed" directory.
saveRDS(items_clean, file = here("data", "processed", "all_items", "three_samples_all_items_no_careless.Rds"))
